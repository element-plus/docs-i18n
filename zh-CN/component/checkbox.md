...
| props ^(2.11.2)             | options 的配置                                                   | ^[object]`{ value?: string, label?: string, disabled?: string}` | `{value: 'value', label: 'label', disabled: 'disabled'}` |
| type ^(2.11.5)              | 用于渲染选项的组件类型（例如 `'button'`）                                    | ^[enum]`'checkbox' \| 'button'`                                | 'checkbox'                                               |
